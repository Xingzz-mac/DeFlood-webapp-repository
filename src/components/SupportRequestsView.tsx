...
export default function SupportRequestsView({ role }: { role: Role }) {
  ...
  const filtered = [...requests].sort((a, b) => Date.parse(b.createdAt) - Date.parse(a.createdAt)).filter(request =>
    (status === 'all' || (status === 'active' ? request.status !== 'RESOLVED' : request.status === status)) &&
  ...
  return <div className="mx-auto max-w-7xl space-y-5 p-4 md:p-6">
    ...
    <div className="flex flex-wrap gap-3">
      <label className="text-sm">Status<select aria-label="Request status filter" value={status} onChange={e => setStatus(e.target.value)} className="ml-2 rounded-lg border p-2"><option value="active">Active</option><option value="all">All</option>{(['PENDING','ACCEPTED','IN_PROGRESS','RESOLVED'] as const).map(s => <option key={s} value={s}>{supportRequestStatusLabel(s)}</option>)}</select></label>
    ...
    {view === 'map' ? <Suspense fallback={<p>Loading request map…</p>}><SupportRequestsMap requests={filtered} onOpen={id => { setSelectedId(id); setView('requests') }} /></Suspense> :
      <div className="grid items-start gap-5 lg:grid-cols-2">
        <section aria-label="Submitted requests" className="space-y-3">
          {!filtered.length && <p className="rounded-xl border border-dashed p-6 text-gray-500">No requests match these filters. Choose All to include resolved requests.</p>}
