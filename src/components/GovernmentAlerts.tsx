...
export default function GovernmentAlerts({ role, candidates }: { role: Role; candidates: AlertCandidate[] }) {
  ...
  return <section aria-label="Government alerts" className="mx-auto max-w-7xl space-y-6 p-4 md:p-6">
    ...
    <section aria-label="Recent Alerts" className="rounded-2xl border border-gray-200 bg-white p-4 md:p-5"><h2 className="text-lg font-bold text-[#1e3a5f]">Recent Alerts</h2>{!alerts.length && <p className="mt-2 text-sm text-gray-500">No alerts issued in this browser.</p>}{alerts.slice(0, 10).map(a => <article key={a.id} className="mt-3 rounded-lg border p-3"><strong>{a.type} · {a.severity}</strong><p className="text-xs text-gray-500">Issued · {new Date(a.issuedAt).toLocaleString()}</p><p className="text-sm">{a.targets.map(t => t.name).join(', ')}</p><details className="mt-2 text-sm"><summary className="cursor-pointer text-blue-800">View message</summary><p className="mt-2 whitespace-pre-wrap break-words">{a.message}</p></details></article>)}</section>
    <p className="text-xs text-gray-500">Prototype alert delivery. No real emergency channels are contacted.</p>
  </section>
}
