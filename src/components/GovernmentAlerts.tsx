...
export default function GovernmentAlerts({ role, candidates }: { role: Role; candidates: AlertCandidate[] }) {
  ...
  return <section aria-label="Government alerts" className="mb-6 space-y-4 rounded-2xl border border-gray-200 bg-white p-4 md:p-5">
    <header><h2 className="text-lg font-bold text-[#1e3a5f]">Communities Requiring Attention</h2><p className="font-semibold text-red-700">{high.length} High-Risk Communities Detected</p><p className="mt-1 text-sm text-gray-600">DeFlood recommends reviewing targeted flood alerts for these communities.</p><p className="mt-2 text-xs text-gray-500">Prototype alert delivery — stored in this browser, not sent to real emergency channels. DeFlood identifies and recommends; authorized officials decide and issue the alert.</p></header>
    ...
    <div className="flex flex-wrap gap-3"><button type="button" disabled={!high.length} className="rounded-lg border px-4 py-2 text-sm disabled:opacity-50" onClick={() => review(high.map(c => alertCommunityId(c.community)))}>Review All HIGH-Risk Communities</button><button type="button" className="rounded-lg border px-4 py-2 text-sm" onClick={() => review([])}>Create Targeted Alert</button></div>
    {targets !== null && <section aria-label="Review government alert" className="space-y-4 rounded-xl border border-blue-200 bg-blue-50 p-4">
      ...
      <div className="flex flex-wrap gap-3">{!preview ? <button type="button" disabled={!recipients.length || !message.trim()} onClick={() => setPreview(true)} className="rounded-lg bg-[#1e3a5f] px-4 py-2 text-white disabled:opacity-50">Preview Alert</button> : <><button type="button" onClick={() => { try { const issued = issueGovernmentAlert(role, { type, severity, message, targets: recipients.map(c => ({ id: alertCommunityId(c.community), name: c.community.name })) }); setAlerts(loadGovernmentAlerts()); setTargets(null); setFeedback(`Alert Issued Successfully — ${issued.type} issued to ${issued.targets.length} selected communities. Prototype delivery only.`) } catch (e) { setFeedback(e instanceof Error ? e.message : 'Alert could not be issued.') } }} className="rounded-lg bg-red-700 px-4 py-2 text-white">Send Alert</button><button type="button" onClick={() => setPreview(false)}>Edit Alert</button></>}<button type="button" onClick={() => setTargets(null)}>Cancel</button></div>
    </section>}
    {feedback && <p role="status" className="rounded-lg border p-3 text-sm">{feedback}</p>}
    <section aria-label="Recent Alerts"><h3 className="font-bold">Recent Alerts</h3>{!alerts.length && <p className="text-sm text-gray-500">No alerts issued in this browser.</p>}{alerts.slice(0, 10).map(a => <article key={a.id} className="mt-3 rounded-lg border p-3"><strong>{a.type} · {a.severity}</strong><p className="text-xs text-gray-500">Issued · {new Date(a.issuedAt).toLocaleString()} · Prototype delivery</p><p className="text-sm">{a.targets.map(t => t.name).join(', ')}</p><p className="whitespace-pre-wrap break-words text-sm">{a.message}</p></article>)}</section>
  </section>
}
