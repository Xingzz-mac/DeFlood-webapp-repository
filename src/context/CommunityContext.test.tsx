...
describe('CommunityContext sample-data provenance', () => {
  ...
  it('labels initial seeded values as demo data and confirms them only after the existing save action', async () => {
    ...
    expect(pageText(renderer!.toJSON())).toContain('Sample workspace — Ayeyarwady Delta Zone 3 is starter data only.')
    ...
    expect(pageText(renderer!.toJSON())).not.toContain('Sample workspace')
